...
2.  [Como os robôs navegam](#como-os-robôs-navegam)
3.  [Nav2](#nav2)
4.  [Mapa](#mapa)
    *   [SLAM](#slam)
    *   [Iniciando o Cartographer](#iniciando-o-cartographer)
        *   [cartographer_node](#cartographer_node)
        *   [occupancy_grid_node](#occupancy_grid_node)
        *   [Exemplo](#exemplo)
        *   [Salvando o mapa](#salvando-o-map)
            *   [O arquivo YAML](#o-arquivo-yaml)
            *   [O arquivo de imagem (PGM)](#o-arquivo-de-imagem-pgm---portable-gray-map)
    *   [Fornecendo o mapa para outros nodes](#fornecendo-o-mapa-para-outros-nodes)
        *   [map_server node](#map_server-node)
        *   [lifecycle_manager node](#lifecycle_manager-node)
5.  [Lifecycle Nodes](#lifecycle-nodes)
    *   [Gerenciador do Nav2 Lifecycle](#gerenciador-do-nav2-lifecycle)
6.  [Como localizar um robô em um ambiente](#como-localizar-um-robô-em-um-ambiente)
    *   [AMCL](#amcl)
    *   [Iniciando o AMCL com um arquivo launch](#iniciando-o-amcl-com-um-arquivo-launch)
        *   [Inicialização do nó map_server](#inicialização-do-nó-map_server)
        *   [Inicialização do nó amcl](#inicialização-do-nó-amcl)
        *   [Inicialização do nó lifecycle_manager](#inicialização-do-nó-lifecycle_manager)
        *   [Manually provide the initial localization](#manually-provide-the-initial-localization)
        *   [Como definir a localização inicial do robô a partir do arquivo de configuração](#como-definir-a-localização-inicial-do-robô-a-partir-do-arquivo-de-configuração)
        *   [Como definir a localização inicial do robô a partir da linha de comando](#como-definir-a-localização-inicial-do-robô-a-partir-da-linha-de-comando)
        *   [Como definir a localização inicial do robô programaticamente](#como-definir-a-localização-inicial-do-robô-programaticamente)
    *   [Localização global](#localização-global)
7.  [Planejamento de trajetórias (Path Planning)](#planejamento-de-trajetórias-path-planning)
8.  [Iniciando (launch) Path Planning](#iniciando-launh-path-planning)
    *   [Iniciando o planejador de rotas globais (planner_server)](#iniciando-o-planejador-de-rotas-globais-planner_server)
    *   [Iniciando o controlador (controller_server)](#iniciando-o-controlador-controller_server)
        *   [Progress Checker](#progress-checker)
        *   [Goal Checker](#goal-checker)
    *   [Iniciando o bt_navigator](#iniciando-o-bt_navigator)
    *   [Iniciando recoveries_server](#iniciando-recoveries_server)
    *   [Enviando uma meta de navegação por linha de comando](#enviando-uma-meta-de-navegação-por-linha-de-comando)
        *   [Com um servidor de ações](#com-um-servidor-de-ações)
        *   [Com um tópico](#com-um-tópico)
9.  [Desviando de obstáculos](#desviando-de-obstáculos)
    *   [Mapa de custo (Costmap)](#mapa-de-custo-costmap)
    *   [Adicionando um mapa de custo global na navegação](#adicionando-um-mapa-de-custo-global-na-navegação)
        *   [Obstacle layers (camadas)](#obstacle-layers-camadas)
    *   [Adicionando um mapa de custo local na navegação](#adicionando-um-mapa-de-custo-local-na-navegação)
10. [Navegação multi robo](#navegação-multi-robo)
    *   [Mapeamento do ambiente](#mapeamento-do-ambiente)
    *   [Localizando](#localizando)
        *   [PASSO 1 - Adicionar uma variável de namespace aos launch de nós amcl](#passo-1---adicionar-uma-variável-de-namespace-aos-launch-de-nós-amcl)
        *   [PASSO 2 - Crie um arquivo de configuração específico para cada robô](#passo-2---crie-um-arquivo-de-configuração-específico-para-cada-robô)
        *   [Configurando o rviz para múltiplos robôs](#configurando-o-rviz-para-multiplos-robos)
    *   [Pathing Planning](#pathing-planning)
        *   [PASSO 1 - Adicionando o nameSpace](#passo-1---adicionando-o-namespace)
        *   [PASSO 2 - Crie um arquivo de configuração específico para cada robô](#passo-2---crie-um-arquivo-de-configuração-específico-para-cada-robô-1)
        *   [Configurando o rviz para múltiplos robôs](#configurando-o-rviz-para-multiplos-robos-1)
    *   [Executando com apenas uma launch](#executando-com-apenas-uma-launch)
