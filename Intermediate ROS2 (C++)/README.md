...
1.  [Launch](#launch)
    *   [Arquivos de launch usando python](#arquivos-de-launch-usando-python)
    *   [Arquivos de launch usando XML](#arquivos-de-launch-usando-xml)
    *   [Arquivos de launch usando YAML](#arquivos-de-launch-usando-yaml)
    *   [LogInfo](#loginfo)
    *   [Executando várias launch a partir de uma unica launch](#executando-várias-launch-a-partir-de-uma-unica-launch)
    *   [Passando parâmetros em arquivos de inicialização](#passando-parâmetros-em-arquivos-de-inicialização)
2.  [Parâmetros de Nodes](#parametros-de-nodes)
    *   [Como os parâmetros funcionam no ROS 2?](#como-os-parâmetros-funcionam-no-ros-2)
    *   [Interaja com parâmetros das ferramentas de linha de comando](#interaja-com-parâmetros-das-ferramentas-de-linha-de-comando)
    *   [Carregando e despejando parâmetros de arquivos YAML](#carregando-e-despejando-parâmetros-de-arquivos-yaml)
    *   [Configurando parâmetros via linha de comando na inicialização do nó](#configurando-parâmetros-via-linha-de-comando-na-inicialização-do-nó)
    *   [Carregando parâmetros no arquivos launch](#carregando-parametros-no-arquivos-launch)
    *   [Parâmetros de callBack](#parâmetros-de-callback)
3.  [Gerenciando nós complexos com C++](#gerenciando-nós-complexos-com-c)
    *   [Uso de Executores](#uso-de-executores)
    *   [Uso de Callback Groups](#uso-de-callback-groups)
    *   [Uso de WaitSet](#uso-de-waitset)
        *   [Otimizando o uso do CPU](#otimizando-o-uso-do-cpu)
        *   [GUARD CONDITIONS](#guard-conditions)
4.  [Qualidade de serviço (QoS)](#qualidade-de-serviço-qos)
    *   [Entendendo a compatibilidade QoS](#entendendo-a-compatibilidade-qos)
    *   [Criando um subscriber](#criando-um-subscriber)
    *   [Políticas de QoS padrão](#políticas-de-qos-padrão)
    *   [Durability](#durability)
    *   [History](#history)
    *   [Depth](#depth)
    *   [Reliability](#reliability)
    *   [Deadline](#deadline)
        *   [Teste diferentes valores de prazo para observar os diferentes comportamentos](#teste-diferentes-valores-de-prazo-para-observar-os-diferentes-comportamentos)
    *   [Lifespan](#lifespan)
        *   [Vida útil do Publisher muito restritiva](#vida-útil-do-publisher-muito-restritiva)
    *   [Liveliness and LeaseDuration](#liveliness-and-leaseduration)
        *   [Usando AUTOMATIC LIVELINESS](#usando-automatic-liveliness)
        *   [Usando o método ALIVE para o MANUAL_BY_TOPIC](#usando-o-método-alive-para-o-manual_by_topic)
        *   [Event Callbacks](#event-callbacks)
    *   [QoS no ROS 2 Bags](#qos-no-ros-2-bags)
        *   [Gravação ROSbags QoS](#gravação-rosbags-qos)
        *   [Replay Changing the QoS](#replay-changing-the-qos)
5.  [DDS](#dds)
    *   [Então, DDS é a mesma coisa que ROS?](#então-dds-é-a-mesma-coisa-que-ros)
    *   [Fundamentos do DDS](#fundamentos-do-dds)
        *   [1. Centralidade de Dados](#1-centralidade-de-dados)
        *   [2. QoS](#2-qos)
        *   [3. Descoberta Dinâmica](#3-descoberta-dinâmica)
        *   [4. Segurança](#4-segurança)
    *   [Então, o DDS é usado no ROS1?](#então-o-dds-é-usado-no-ros1)
    *   [Verifique o Middleware atual](#verifique-o-middleware-atual)
    *   [Implementações de DDS suportadas no Galactic](#implementações-de-dds-suportadas-no-galactic)
        *   [Eclipse Cyclone](#eclipse-cyclone)
    *   [Como alterar a implementação do DDS](#como-alterar-a-implementação-do-dds)
    *   [O que é o ROS 2 Daemon](#o-que-é-o-ros-2-daemon)
6.  [LifecycleNode](#lifecyclenode)
    *   [Por que você precisa de nós gerenciados?](#por-que-você-precisa-de-nós-gerenciados)
    *   [O que é um nó gerenciado?](#o-que-é-um-nó-gerenciado)
    *   [Exemplo simples de nó gerenciado](#exemplo-simples-de-nó-gerenciado)
    *   [Interface de linha de comando (CLI) de nós gerenciados](#interface-de-linha-de-comando-cli-de-nós-gerenciados)
    *   [Nenhuma publicação no tópico](#nenhuma-publicação-no-tópico)
    *   [O resultado de cada função de transição de estado](#o-resultado-de-cada-função-de-transição-de-estado)
        *   [Parando o nó](#parando-o-nó)
        *   [Pausando o nó](#pausando-o-nó)
        *   [Desativando o nó e reativando (reiniciando)](#desativando-o-nó-e-reativando-reiniciando)
        *   [Exceções não capturadas](#exceções-não-capturadas)
    *   [Life Cycle Manager](#life-cycle-manager)
        *   [1. Crie o cliente de serviço](#1-crie-o-cliente-de-serviço)
        *   [2. Chame o cliente para fazer uma transição](#2-chame-o-cliente-para-fazer-uma-transição)
    *   [Nós gerenciados no Nav2](#nós-gerenciados-no-nav2)
...
* o managed_scan_node. Este nó é gerenciado durante a publicação de dados de laser.
* o manager_client_node. Este nó faz a transição do nó anterior para um estado ativo.

**Vamos ver como esse gerenciador funciona:**

### 1. Crie o cliente de serviço
Seu gerenciador se comunicará com o serviço fornecido pelos nós gerenciados por padrão. É assim que o gerenciador altera os estados dos nós conforme sua vontade.

* O gerenciador utilizará o serviço /managed_scan_node/change_state.
* O gerenciador deve então incluir um cliente de serviço, como você estudou no curso ROS2 Basics C++.

O cliente é criado na seguinte linha:
...
Lembre-se de que as mensagens de estado que você vai usar são:
...
O Nav2 utiliza extensivamente nós gerenciados.

O Nav2 utiliza um wrapper do LifecycleNode, o **nav2_util**, e o **LifecycleNode**. Este wrapper oculta muitas das complexidades dos LifecycleNodes para aplicações típicas. Ele também inclui uma conexão de vínculo para o gerenciador de ciclo de vida, garantindo que ele permaneça ativo após a transição de um nó para o nível superior. Quando um nó falha, ele informa o gerenciador de ciclo de vida e faz a transição do sistema para o nível inferior para evitar uma falha crítica.
